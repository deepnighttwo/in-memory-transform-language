 ...
 * Time: 18:03
 */
public class ReflectPerfTestUseRetBoxLongLogic {
    ...
    static long directCall(long count) {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                sum += data.perfTestUseRetLongContainsLogic();
            }
    ...
    static long reflectCall() {
        ...
        try {
            PerfTestBean data = new PerfTestBean();
            Method method = PerfTestBean.class.getMethod("perfTestUseRetLongContainsLogic");
            long start = System.currentTimeMillis();
            for (int i = 0; i < count; i++) {
